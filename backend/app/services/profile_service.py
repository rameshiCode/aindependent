...
from app.models import ProfileAttributeModel, UserProfileModel
...
class ProfileService:
    ...
    def get_profile(self, user_id: str) -> UserProfileModel:
        ...
        try:
            ...
            profile = UserProfileModel(
                user_id=user_id, created_at=datetime.now(), last_updated=datetime.now()
            )
    ...
    def save_profile(self, profile: UserProfileModel) -> bool:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error saving profile: {str(e)}"
            )
    ...
    def update_attribute(
        self, user_id: str, attr_name: str, value: Any, confidence: float
    ) -> bool:
        ...
        if not hasattr(profile, attr_name):
            raise HTTPException(
                status_code=400, detail=f"Attribute {attr_name} does not exist"
            )
        ...
        if current_attr is None or current_attr.confidence < confidence:
            ...
            new_attr = ProfileAttributeModel(
                value=value, confidence=confidence, last_updated=datetime.now()
            )
